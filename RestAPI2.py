...
username = getpass.getpass(prompt="Enter the username...")
...
response = requests.post('https://localhost:8089/servicesNS/admin/search/search/jobs', data=data, verify=False, auth=(username, password))
print(response.text)
...
while (is_job_completed != 'DONE'):
    ...
    resp_job_status = requests.post(splunk_status_base_url, data=data, verify=False, auth=(username, password))
...
splunk_base_results = requests.get(splunk_base_url, data=data, verify=False, auth=(username, password))
